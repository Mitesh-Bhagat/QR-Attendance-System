...
if (isset($_GET['data']) && isset($_GET['lat']) && isset($_GET['lon'])) {
    ...
    if (($currentTimestamp - $qrTimestamp) > 3600) {
        $_SESSION['msg'] = '<div class="alert alert-danger glass-alert mb-2" role="alert"><i class="fas fa-exclamation-circle me-2"></i> QR code has expired!.</div>';
    ...
    if ($distance > $allowedDistance) {
        $_SESSION['msg'] = '<div class="alert alert-danger glass-alert mb-2" role="alert"><i class="fas fa-map-marker-alt me-2"></i> You are not within the allowable distance (' . $allowedDistance . 'km) to mark attendance.</div>';
        header("location: give_attend.php");
        exit();
    }
                        ...
                        if ($ires->num_rows == 0) {
                            ...
                            if ($cres->num_rows == 0) {
                                ...
                                if ($fres) {
                                    ...
                                    $_SESSION['msg'] = '<div class="alert alert-success glass-alert mb-2" role="alert"><i class="fas fa-check-circle me-2"></i> Attendance Marked Successfully!.</div>';
                                    header("location: give_attend.php");
                                    exit();
                                } else {
                                    $_SESSION['msg'] = '<div class="alert alert-danger glass-alert mb-2" role="alert"><i class="fas fa-times-circle me-2"></i> Database Error!.</div>';
                                    header("location: give_attend.php");
                                    exit();
                                }
                            } else {
                                $_SESSION['msg'] = '<div class="alert alert-warning glass-alert mb-2" role="alert"><i class="fas fa-exclamation-triangle me-2"></i> Attendance Already Marked!.</div>';
                                header("location: give_attend.php");
                                exit();
                            }
                        } else {
                            $_SESSION['msg'] = '<div class="alert alert-danger glass-alert mb-2" role="alert"><i class="fas fa-network-wired me-2"></i> One IP Address can only give one attendance.</div>';
                            header("location: give_attend.php");
                            exit();
                        }
                    } else {
                        $_SESSION['msg'] = '<div class="alert alert-danger glass-alert mb-2" role="alert"><i class="fas fa-user-slash me-2"></i> This Branch/Batch/Semester is not allocated to you!.</div>';
                        header("location: give_attend.php");
                        exit();
                    }
                } else {
                    $_SESSION['msg'] = '<div class="alert alert-danger glass-alert mb-2" role="alert"><i class="fas fa-globe-asia me-2"></i> Only Indian IP Addresses Allowed.</div>';
                    header("location: give_attend.php");
                    exit();
                }
            } else {
                $_SESSION['msg'] = '<div class="alert alert-danger glass-alert mb-2" role="alert"><i class="fas fa-shield-alt me-2"></i> VPN and Proxy Usage Detected. Please disable them.</div>';
                header("location: give_attend.php");
                exit();
            }
        }
    } else {
        $_SESSION['msg'] = '<div class="alert alert-danger glass-alert mb-2" role="alert"><i class="fas fa-server me-2"></i> VPN & Proxy Check Failed. API Error.</div>';
        header("location: give_attend.php");
        exit();
    }
} else {
    $_SESSION['msg'] = '<div class="alert alert-danger glass-alert mb-2" role="alert"><i class="fas fa-map-marked-alt me-2"></i> QR or Location Data Missing. Please refresh and try again.</div>';
    header("location: give_attend.php");
    exit();
}
?>